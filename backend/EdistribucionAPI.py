...
class Edistribucion():
    ...
    def get_maximeter_histogram(self, cups):

        today = datetime.today()
        dStart = str(today.month) + '/' + str(today.year - 2)
        dEnd = str(today.month) + '/' + str(today.year)
        data = {
            'message': '{"actions":[{"id":"490;a","descriptor":"apex://WP_MaximeterHistogram_CTRL/ACTION$getHistogramPoints","callingDescriptor":"markup://c:WP_MaximeterHistogramDetail","params":{"mapParams":{"startDate":"'+ dStart +'","endDate":"' + dEnd + '","id":"'+cups+'","sIdentificador":"'+self.__identities['account_id']+'"}}}]}',
            }
        r = self.__command('other.WP_MaximeterHistogram_CTRL.getHistogramPoints=1', post=data)
        return r

    def get_chart_points_by_range(self, cont, sStart, sEnd):

        data = {
            'message': '{"actions":[{"id":"949;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getChartPointsByRange","callingDescriptor":"markup://c:WP_Measure_Detail_Filter_Advanced_v3","params":{"contId":"'+cont['Id']+'","type":"4","startDate":"' + sStart + '","endDate":"' + sEnd + '"},"version":null,"longRunning":true}]}',
            }
